...
Common functionality for all table migration scripts.
Provides database connections, validation patterns, and error handling.
"""
...
class BaseMigrator:
    """Base class for table migration scripts."""
    
    def __init__(self, table_name: str, v1_db_name: str = "blub_test", v2_db_name: str = "blub_test_v2", model_name: str = None):
        self.table_name = table_name
        self.v1_db_name = v1_db_name
        self.v2_db_name = v2_db_name
        
        # Use testing environment for proper DI and config
        self.testing_env = get_testing_environment(model_name=model_name, log_level="INFO")
        self.config = self.testing_env.get_config()
        
        # Set up simple print-based logging for migration scripts
    ...
    def _setup_database_connections(self):
        ...
        print(f"🔗 Setting up database connections for {self.table_name} migration...")
        
        # Build connection URLs for both databases
        base_url = f"postgresql://{self.config.db_user}:{self.config.db_password}@{self.config.db_host}:{self.config.db_port}"
