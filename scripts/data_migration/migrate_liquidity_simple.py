#!/usr/bin/env python3
"""
Simple Liquidity Table Migration Script

Migrates liquidity table data from v1 to v2 database using direct database connections.
This version bypasses the DI container to avoid configuration issues.
"""
...
from scripts.data_migration.simple_base_migrator import SimpleBaseMigrator


class SimpleLiquidityMigrator(SimpleBaseMigrator):
    """Migrate liquidity table from v1 to v2 database."""
    
    def __init__(self, v1_db_name: str = "blub_test", v2_db_name: str = "blub_test_v2"):
        super().__init__("liquidity", v1_db_name, v2_db_name)
...
def main():
    ...
    migrator = SimpleLiquidityMigrator(v1_db_name=args.v1_db, v2_db_name=args.v2_db)
