#!/usr/bin/env python3
"""
Hybrid Base Migration Class

Uses DI container for database credentials (GCS secrets) but bypasses model configuration issues.
Creates direct database connections using the container's database config.
"""
...
class HybridBaseMigrator:
    """Base class for table migration scripts using DI for credentials but direct DB connections."""
    ...
    def __init__(self, table_name: str, v1_db_name: str = "blub_test", v2_db_name: str = "blub_test_v2"):
        ...
        # Get database credentials using the secrets service (bypassing full model config)
        self._get_db_config_from_container()
        self._setup_database_connections()
        
    def _get_db_config_from_container(self):
        """Get database configuration using DI container for credentials."""
        print("🔑 Getting database credentials from GCS...")
        
        try:
            # Get project ID from environment variable
            project_id = os.getenv('INDEXER_GCP_PROJECT_ID')
            if not project_id:
                raise ValueError("INDEXER_GCP_PROJECT_ID environment variable not set")
            
            # Create a secrets service to get database credentials
            ...
            print(f"   ✅ Credentials retrieved from GCS")
            print(f"   DB Host: {self.db_host}:{self.db_port}")
            print(f"   DB User: {self.db_user}")
            
        except Exception as e:
            print(f"   ❌ Failed to get credentials from GCS: {e}")
            # Fallback to environment variables
            print("   🔄 Falling back to environment variables...")
            self._get_db_config_from_env()
    
    def _get_db_config_from_env(self):
        """Fallback to environment variables if GCS secrets fail."""
        ...
        if not self.db_password:
            raise ValueError(
                "Database password not found. Either fix GCS secrets access or set "
                "INDEXER_DB_PASSWORD/DB_PASSWORD environment variable"
            )
