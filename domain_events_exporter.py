#!/usr/bin/env python3
"""
DI-Integrated Domain Events Exporter

Uses your dependency injection container to properly initialize all services
and export domain event data to CSV files. This follows your established
patterns and integrates with your existing infrastructure.
"""
...
class DomainEventsExporter:
    """Export domain events using your DI container infrastructure"""
    
    def __init__(self, model_name: str = None, output_dir: str = "domain_events_export"):
        """
        Initialize exporter with DI container
        ...
        self.logger = IndexerLogger.get_logger('tools.domain_events_exporter')
        
        # Domain event tables definition from your schema
        self.domain_event_tables = {
            'trades': {
                'description': 'Trade events - user trading activity',
                'key_columns': ['taker', 'direction', 'base_token', 'base_amount', 'trade_type'],
                'relationships': ['Links to pool_swaps via content_id references']
            },
            'pool_swaps': {
                'description': 'Individual pool swap events within trades',
                'key_columns': ['pool', 'taker', 'direction', 'base_token', 'quote_token'],
    ...
    def _initialize_container(self):
        """Initialize the DI container and get required services"""
        try:
            log_with_context(self.logger, logging.INFO, "Initializing DI container",
                           model_name=self.model_name)
            
            # Create indexer with full DI container
            ...
            log_with_context(self.logger, logging.INFO, "DI container initialized successfully",
                           model_name=self.config.model_name,
                           model_version=self.config.model_version,
                           model_db=self.config.model_db_name)
            ...
            log_with_context(self.logger, logging.ERROR, "Failed to initialize DI container",
                           error=str(e), model_name=self.model_name)
            raise
    
    def get_table_info(self, table_name: str) -> Dict[str, Any]:
        """Get table metadata and statistics using your database manager"""
        try:
    ...
    def export_table(self, table_name: str, limit: Optional[int] = None) -> bool:
        ...
        try:
            ...
            print(f"📊 Exporting {table_name} ({table_info['row_count']:,} rows)...")
            ...
            # Build query with smart ordering based on your schema
            ...
            # Export data using your database manager
            with self.model_db.get_session() as session:
            ...
            if len(df) > 0:
                ...
                # Show sample key data from your schema
    ...
    def export_all_domain_events(self, limit_per_table: Optional[int] = None):
        """Export all domain event tables"""
    ...
    def export_processing_context(self, limit_per_table: Optional[int] = 1000):
    ...
    def run_analysis_queries(self):
        """Run analysis queries using your database connection"""
        ...
        for query_config in analysis_queries:
            try:
                ...
                if not df.empty:
                    ...
                    # Show preview of summary data
                    if len(df) > 0 and query_config['name'] == 'domain_events_summary':
                        print(f"      📋 Preview:")
    ...
    def generate_export_report(self):
        ...
        try:
            ...
            with open(report_path, 'w') as f:
                ...
                for file_path in sorted(self.output_dir.glob("*.csv")):
...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        print("Example: python domain_events_exporter.py blub_test 5000")
    ...
    print("🚀 DI-Integrated Domain Events Exporter")
    print("=" * 60)
    print(f"Model: {model_name}")
    print(f"Limit: {limit_per_table or 'No limit'}")
    print()
    
    try:
        # Create exporter using your DI infrastructure
        exporter = DomainEventsExporter(
            model_name=model_name,
            output_dir=f"db_exporter/blub_test"
        )
        ...
        exporter.export_processing_context(limit_per_table=1000)
        ...
        if success:
            ...
            print(f"📁 Check the output directory: {exporter.output_dir}")
