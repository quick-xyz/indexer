# indexer/transform/transformers/routers/base_router.py
...
class BaseRouterTransformer(BaseTransformer):
    """
    Base transformer for router and aggregator contracts.
    
    Can operate in two modes:
    1. Router Mode: Aggregates only PoolSwap events into Trade events
    2. Aggregator Mode: Aggregates both PoolSwap and Trade events into higher-level Trade events
    
    This base class provides the core aggregation logic that can be reused
    by specific router transformers.
    """
    
    def __init__(self, contract: EvmAddress, wnative: EvmAddress, 
                 aggregation_mode: str = "router", trade_type: str = "trade"):
        """
        Initialize router transformer.
        
        Args:
            contract: Router/aggregator contract address
            wnative: Wrapped native token address (for base/quote determination)
            aggregation_mode: "router" (PoolSwaps only) or "aggregator" (PoolSwaps + Trades)
            trade_type: Type of trade to create ("trade", "arbitrage", etc.)
        """
        super().__init__(contract_address=contract.lower())
        self.wnative = wnative.lower()
