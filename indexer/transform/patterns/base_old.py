...
class TransferPattern(ABC):
    ...
    def _match_transfers(self, legs: List[TransferLeg], unmatched_transfers: TransfersDict) -> Optional[Dict[int,TransferSignal]]:
        ...
        for leg in legs:
            # Look for transfers in the "out" direction from from_end
            if leg.token in unmatched_transfers:
                outgoing_from_source = unmatched_transfers[leg.token]["out"].get(leg.from_end, {})
                
                # Find transfers that go to the correct destination
                for transfer_id, transfer in outgoing_from_source.items():
                    if transfer.to_address == leg.to_end:
                        matched_transfers[transfer_id] = transfer
                        break
        
        return matched_transfers if matched_transfers else None
