...
from sqlalchemy import Column, TIMESTAMP, Enum, Index
...
class DBPriceVwap(SharedBase, SharedTimestampMixin):
    ...
    @classmethod
    def get_canonical_price(cls, session, asset_address: str, timestamp, denom: PricingDenomination = PricingDenomination.USD):
    ...
    @classmethod
    def get_price_before_timestamp(cls, session, asset_address: str, timestamp, denom: PricingDenomination = PricingDenomination.USD):
        minute_timestamp = func.date_trunc('minute', timestamp)
    ...
    @classmethod
    def get_price_range(cls, session, asset_address: str, start_time, end_time, denom: PricingDenomination = PricingDenomination.USD):
        return session.query(cls).filter(
    ...
    @classmethod
    def get_latest_price(cls, session, asset_address: str, denom: PricingDenomination = PricingDenomination.USD):
        return session.query(cls).filter(
            cls.asset == asset_address,
            cls.denom == denom
        ).order_by(cls.time.desc()).first()