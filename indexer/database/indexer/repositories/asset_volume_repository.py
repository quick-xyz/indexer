    ...
    def get_volume_for_period(
        ...
        try:
            return session.query(AssetVolume).filter(
                ...
                AssetVolume.asset == asset_address.lower(),    # ✅ Correct: Table uses 'asset'
                AssetVolume.denom == denom.lower()             # ✅ Correct: Table uses 'denom'
            ).all()
    ...
    def get_protocol_volume_range(
        ...
        try:
            return session.query(AssetVolume).filter(
                ...
                AssetVolume.asset == asset_address.lower(),    # ✅ Correct: Table uses 'asset'
                AssetVolume.protocol == protocol.lower(),      # ✅ Correct: Table uses 'protocol'
                AssetVolume.denom == denom.lower()             # ✅ Correct: Table uses 'denom'
    ...
    def update_volume_record(
        ...
        try:
            ...
            volume_record = session.query(AssetVolume).filter(
                AssetVolume.period_id == period_id,
                AssetVolume.asset == asset.lower(),           # ✅ Correct: Table uses 'asset'
                AssetVolume.denom == denom.lower(),           # ✅ Correct: Table uses 'denom'
                AssetVolume.protocol == protocol.lower()      # ✅ Correct: Table uses 'protocol'
            ).first()
        ...
        try:
            return session.query(AssetVolume).filter(
                and_(
                    AssetVolume.period_id == period_id,
                    AssetVolume.asset == asset_address.lower(),     # ✅ Correct: Table uses 'asset'
                    AssetVolume.denom == denomination.lower(),      # ✅ Correct: Table uses 'denom'
                    AssetVolume.protocol == protocol.lower()        # ✅ Correct: Table uses 'protocol'
                )
            ).one_or_none()
