# indexer/database/model/repositories/asset_price_repository.py
...
from ...model.tables import DBAssetPrice
...
class AssetPriceRepository(BaseRepository):
    ...
    def __init__(self, db_manager: ModelDatabaseManager):
        super().__init__(db_manager, DBAssetPrice)
    ...
    def get_candle(
        ...
        try:
            return session.query(DBAssetPrice).filter(
                and_(
                    DBAssetPrice.period_id == period_id,
                    DBAssetPrice.asset == asset_address.lower(),    # ✅ Fixed: Table uses 'asset'
                    DBAssetPrice.denom == denomination.value       # ✅ Fixed: Table uses 'denom'
                )
            ).one_or_none()
    ...
    def get_candles_for_periods(
        ...
        try:
            return session.query(DBAssetPrice).filter(
                and_(
                    DBAssetPrice.period_id.in_(period_ids),
                    DBAssetPrice.asset == asset_address.lower(),    # ✅ Fixed: Table uses 'asset'
                    DBAssetPrice.denom == denomination.value       # ✅ Fixed: Table uses 'denom'
                )
            ).order_by(DBAssetPrice.period_id).all()
    ...
    def get_latest_candle_timestamp(
        ...
        try:
            ...
            latest_period_id = session.query(func.max(DBAssetPrice.period_id)).filter(
                DBAssetPrice.asset == asset_address.lower()    # ✅ Fixed: Table uses 'asset'
            ).scalar()
    ...
    def get_candle_stats(
        ...
        try:
            stats = session.query(
                func.count(DBAssetPrice.period_id).label('candle_count'),
                func.min(DBAssetPrice.period_id).label('earliest_period'),
                func.max(DBAssetPrice.period_id).label('latest_period'),
                func.avg(DBAssetPrice.close).label('avg_close_price'),       # ✅ Fixed: Table uses 'close'
                func.min(DBAssetPrice.low).label('lowest_price'),            # ✅ Fixed: Table uses 'low'
                func.max(DBAssetPrice.high).label('highest_price')           # ✅ Fixed: Table uses 'high'
            ).filter(
                and_(
                    DBAssetPrice.asset == asset_address.lower(),             # ✅ Fixed: Table uses 'asset'
                    DBAssetPrice.denom == denomination.value                 # ✅ Fixed: Table uses 'denom'
