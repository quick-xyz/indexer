# indexer/database/model/repositories/trade_detail_repository.py
...
class TradeDetailRepository(BaseRepository):
    ...
    def __init__(self, db_manager: ModelDatabaseManager):
        super().__init__(db_manager, DBTradeDetail)
    ...
    ) -> Optional[DBTradeDetail]:
        """Create a new trade detail record"""
        try:
            detail = DBTradeDetail(
    ...
    def get_by_content_id(self, session: Session, content_id: DomainEventId) -> List[DBTradeDetail]:
        """Get all pricing details for a trade"""
        try:
            return session.query(DBTradeDetail).filter(
                DBTradeDetail.content_id == content_id
            ).order_by(DBTradeDetail.denom).all()                    # ✅ Fixed: Correct field name
        except Exception as e:
            log_with_context(
    ...
    def get_by_content_id_and_denom(
        ...
        try:
            return session.query(DBTradeDetail).filter(
                and_(
                    DBTradeDetail.content_id == content_id,
                    DBTradeDetail.denom == denom                      # ✅ Fixed: Correct field name
                )
            ).one_or_none()
        except Exception as e:
            log_with_context(
    ...
    def get_by_pricing_method(
        ...
        try:
            return session.query(DBTradeDetail).filter(
                DBTradeDetail.price_method == price_method           # ✅ Fixed: Correct field name
            ).order_by(desc(DBTradeDetail.created_at)).limit(limit).all()
        except Exception as e:
            log_with_context(
    ...
    def get_usd_valuations(self, session: Session, limit: int = 100) -> List[DBTradeDetail]:
        """Get USD valuation details"""
        try:
            return session.query(DBTradeDetail).filter(
                DBTradeDetail.denom == PricingDenomination.USD       # ✅ Fixed: Correct field name
            ).order_by(desc(DBTradeDetail.created_at)).limit(limit).all()
        except Exception as e:
            log_with_context(
    ...
    def get_avax_valuations(self, session: Session, limit: int = 100) -> List[DBTradeDetail]:
        """Get AVAX valuation details"""
        try:
            return session.query(DBTradeDetail).filter(
                DBTradeDetail.denom == PricingDenomination.AVAX      # ✅ Fixed: Correct field name
            ).order_by(desc(DBTradeDetail.created_at)).limit(limit).all()
        except Exception as e:
            log_with_context(
    ...
    def get_trades_in_period(
        ...
        """
        try:
            return session.query(DBTradeDetail).join(
                DBTrade, DBTradeDetail.content_id == DBTrade.content_id
            ).join(
                DBPeriod, 
                and_(
                    DBTrade.timestamp >= DBPeriod.time_open,
                    DBTrade.timestamp <= DBPeriod.time_close
                )
            ).filter(
                and_(
                    DBPeriod.id == period_id,
                    DBTrade.base_token == asset_address.lower(),
                    DBTradeDetail.denom == denomination           # ✅ Fixed: Correct field name
                )
    ...
    def calculate_direct_pricing(
        ...
        try:
            ...
            query = session.query(DBTrade).filter(
                DBTrade.base_token == asset_address.lower()  # Use base_token instead of asset_in
            )
            
            # Add time filter if specified
            if days_back:
                cutoff_time = datetime.now(timezone.utc) - timedelta(days=days_back)
                query = query.filter(DBTrade.timestamp >= cutoff_time)

            # Find trades without direct pricing (no TradeDetail with DIRECT method)
            trades_without_direct_pricing = query.filter(
                ~exists().where(
                    and_(
                        DBTradeDetail.content_id == DBTrade.content_id,
                        DBTradeDetail.price_method == TradePricingMethod.DIRECT  # ✅ Fixed: Correct field name
                    )
                )
            ).all()
            ...
            for trade in trades_without_direct_pricing:
                try:
                    ...
                    swap_details = session.query(DBPoolSwapDetail).join(
                        DBPoolSwap, DBPoolSwapDetail.content_id == DBPoolSwap.content_id
                    ).filter(
                        DBPoolSwap.trade_id == trade.content_id
                    ).all()
    ...
    def get_direct_pricing_stats(
        ...
        try:
            ...
            usd_stats = session.query(
                func.count(DBTradeDetail.content_id).label('detail_count'),
                func.count(case([(DBTradeDetail.price_method == TradePricingMethod.DIRECT, 1)])).label('direct_count'),  # ✅ Fixed: Correct field name
                func.count(case([(DBTradeDetail.price_method == TradePricingMethod.GLOBAL, 1)])).label('global_count'),  # ✅ Fixed: Correct field name
                func.sum(DBTradeDetail.value).label('total_volume'),      # ✅ Fixed: Use actual field
                func.avg(DBTradeDetail.price).label('avg_price')          # ✅ Fixed: Use actual field
            ).join(
                DBTrade, DBTradeDetail.content_id == DBTrade.content_id
            ).filter(
                and_(
    ...
    def get_trades_without_direct_pricing(
        ...
        try:
            return session.query(DBTrade).filter(
                and_(
                    DBTrade.block_number == block_number,
                    DBTrade.base_token == asset_address.lower(),
                    ~exists().where(
                        and_(
                            DBTradeDetail.content_id == DBTrade.content_id,
    ...
    def get_latest_pricing_timestamp(
        ...
        try:
            latest = session.query(func.max(DBTrade.timestamp)).join(
                DBTrade, DBTradeDetail.content_id == DBTrade.content_id
            ).filter(
                DBTrade.base_token == asset_address.lower()
            ).scalar()
